...
<div id="container">
	<h1>Ubah Voucher Pengeluaran Kas/Bank</h1>
    <form id="frm_edit_rvh" method="POST" action="<?php echo base_url().'payment_voucher/edit'; ?>">
	<div id="entry_header">
        <ul>
            <li><label>Nomor Voucher:</label><span style="font-weight: bold;"><?php echo $pvh['PV_Nomor']; ?></span></li>
            <li><label>Tanggal:</label><input title="Isi dengan tanggal voucher" type="text" name="PV_Tanggal" id="PV_Tanggal" maxlength="10" value="<?php echo set_value('PV_Tanggal', date('Y-m-d', strtotime($pvh['PV_Tanggal'])));?>" <?php echo (form_error('PV_Tanggal')!=false) ? 'style="background:#f5c8c8"' : ''; ?> /></li>
            <?php if (form_error('PV_Tanggal')!=false) { ?>
            <li><label></label><?php echo form_error('PV_Tanggal', '<span class="err_text">', '</span>'); ?></li>
            <?php } ?>            
            <li><label>No Referensi:</label><input title="Isi dengan nomor referensi (po/faktur/dll)" type="text" name="PV_Referensi" id="PV_Referensi" maxlength="20" value="<?php echo set_value('PV_Referensi', $pvh['PV_Referensi']);?>" /></li>
            <li><label>Dibayar kepada:</label><select name="PV_Kepada" id="PV_Kepada" style="width:350px;"><?php echo $identitas_list; ?></select></li>
            <li>
                <label></label>
                <span>Identitas (baru) : </span>
                <input type="text" title="Isi dengan kode identitas objek akuntansi" name="PV_Identitas" id="PV_Identitas" maxlength="20" style="width:100px;<?php echo (form_error('PV_Identitas')!=false) ? 'background:#f5c8c8' : ''; ?>" />
                <input type="text" title="Isi dengan nama identitas objek akuntansi" name="PV_Nama_Identitas" id="PV_Nama_Identitas" maxlength="20" style="width:300px;<?php echo (form_error('PV_Nama_Identitas')!=false) ? 'background:#f5c8c8' : ''; ?>" />
            </li>
            <?php if (form_error('PV_Kepada')!=false) { ?>
            <li><label></label><?php echo form_error('PV_Kepada', '<span class="err_text">', '</span>'); ?></li>
            <?php } ?>            
            <li><label>Untuk:</label><input title="Isi dengan keterangan umum transaksi" type="text" name="PV_Keterangan" id="PV_Keterangan" maxlength="100" value="<?php echo set_value('PV_Keterangan', $pvh['PV_Keterangan']);?>" style="width:300px;<?php echo (form_error('PV_Keterangan')!=false) ? 'background:#f5c8c8' : ''; ?>" /></li>
            <?php if (form_error('PV_Keterangan')!=false) { ?>
            <li><label></label><?php echo form_error('PV_Keterangan', '<span class="err_text">', '</span>'); ?></li>
            <?php } ?>            
            <li><label>Jenis Penerimaan:</label><select name="PV_Receipt_Type" style="width:110px;"><option value="Kas">Kas</option><option value="Bank">Bank</option></select></li>
            <li><label>Rekening:</label><select name="PV_Account_Number" style="width:350px;"><?php echo $account_list; ?></select></li>
            <li><label>Kode Unit Usaha:</label><select id="PV_Unit_Code" name="PV_Unit_Code" style="width:200px;"><?php echo $unit_list; ?></select></li>
            <li><label>Proyek/Order:</label><select id="PV_Project" name="PV_Project" style="width:300px;"><?php echo $project_list; ?></select></li>
            <li>
                <label></label>
                <span>Proyek/Order (baru) : </span>
                <input type="text" title="Isi dengan nomor kontrak atau kosongkan jika transaksi bukan transaksi proyek" name="PV_Project_Contract_Number" id="PV_Project_Contract_Number" maxlength="20"  style="width:100px;<?php echo (form_error('PV_Project_Contract_Number')!=false) ? 'background:#f5c8c8' : ''; ?>" />
                <input type="text" title="Isi dengan keterangan kontrak atau kosongkan jika transaksi bukan transaksi proyek" name="PV_Project_Name" id="PV_Project_Name" maxlength="100" style="width:300px;<?php echo (form_error('PV_Project_Name')!=false) ? 'background:#f5c8c8' : ''; ?>" />
            </li>
            <input type="hidden" name="PV_Nomor" value="<?php echo $pvh['PV_Nomor']; ?>" />
        </ul>
    </div>
    <div id="entry_detail">
        <ul class="tb_head">
        ...
        <?php 
            ...
            { $rupiah[$no] = (float)str_replace(',','', $rupiah[$no]); ?>
                <ul id="d_row_<?php echo $i ?>" class="tb_row" style="border-bottom: 1px dotted black; ">
                    ...
                    <li class="tb_col tb_col_nomor"><input type="text" class="no_urut tb_col_nomor" name="no_urut[<?php echo $i ?>]" id="no_urut_<?php echo $i ?>" value="<?php echo $urut ?>" /></li>
                    <li class="tb_col tb_col_referensi"><input type="text" class="referensi tb_col_referensi" name="referensi[<?php echo $i ?>]" id="referensi_<?php echo $i ?>" value="<?php echo $referensi[$no]; ?>" /></li>
                    <li class="tb_col">
                        <div class="tb_col_account" style="display: block; border-left: 1px dotted gray;">      
                            <input type="text" class="tb_col_account" name="account[<?php echo $i ?>]" id="account_<?php echo $i ?>" value="<?php echo $account[$no]; ?>" onchange="check_rekening(<?php echo $i ?>)" />
                            ...
                            <span class="field_info" class="tb_col_account" id="account_name_<?php echo $i ?>"><?php echo $this->account_model->get_account_name($account[$no]); ?></span>
                        </div>
                    </li>
                    <li class="tb_col">
                        <div class="tb_col_unit" style="display: block;  border-left: 1px dotted gray;">
                            <input type="text" class="tb_col_unit" name="unit[<?php echo $i ?>]" id="unit_<?php echo $i ?>" value="<?php echo $unit[$no]; ?>" onchange="check_unit(<?php echo $i ?>)" />
                            ...
                            <span class="field_info" class="tb_col_unit" id="unit_name_<?php echo $i ?>"><?php echo $this->unit_model->get_unit_name($unit[$no]); ?></span>
                        </div>
                    </li>
                    <li class="tb_col">
                        <div class="tb_col_project" style="display: block;  border-left: 1px dotted gray;">
                            <input class="tb_col_project" type="text" name="project[<?php echo $i ?>]" id="project_<?php echo $i ?>" value="<?php echo $project[$no]; ?>" onchange="check_project(<?php echo $i ?>)" />
                    ...
                    <li class="tb_col">
                        <div class="tb_col_identitas" style="display: block;  border-left: 1px dotted gray;">
                            <input type="text" class="tb_col_identitas" name="identitas[<?php echo $i ?>]" id="identitas_<?php echo $i ?>" value="<?php echo $identitas[$no]; ?>" onchange="check_identitas(<?php echo $i ?>)" />
                    ...
                    <li class="tb_col"><input class="rupiah tb_col_nominal" type="text" name="rupiah[<?php echo $i ?>]" id="rupiah_<?php echo $i ?>" value="<?php echo number_format((float)$rupiah[$no], 2, '.', ','); ?>" onblur="hitung_total(<?php echo $i ?>)" onkeydown="return f_onlyNumbers(event)" /></li>                    
                </ul>                 
        ...
        <?php } else { $i=1; $urut=5; $total=0;
            foreach ($rvds as $rvd) { ?>
                <ul id="d_row_<?php echo $i; ?>" class="tb_row" style="border-bottom: 1px dotted black; ">
                    <li class="tb_col" style="width:2px;"></li>
                    <li class="tb_col tb_col_del_icon"><img src="<?php echo base_url().'includes/icons/'; ?>cross_10.png" title="Hapus data" alt="Hapus data" id="rvd_del_<?php echo $i; ?>"  onclick="f_del_rvd('<?php echo $i ?>')" style="cursor:pointer;"/></li>
                    <li class="tb_col tb_col_nomor"><input maxlength="3" type="text" class="no_urut tb_col_nomor" name="no_urut[<?php echo $i; ?>]" id="no_urut_[<?php echo $i; ?>]" value="<?php echo set_value('no_urut['.$i.']', $i * 5); ?>" /></li>
                    <li class="tb_col tb_col_referensi"><input maxlength="20" type="text" class="referensi tb_col_referensi" name="referensi[<?php echo $i; ?>]" id="referensi_<?php echo $i; ?>" value="<?php echo set_value('referensi_['.$i.']', $rvd['PVD_Nomor_Referensi']); ?>" /></li>
                    <li class="tb_col">
                        <div class="tb_col_account" style="display: block; border-left: 1px dotted gray;">
                            <input maxlength="15" type="text" class="tb_col_account" name="account[<?php echo $i; ?>]" id="account_<?php echo $i; ?>" value="<?php echo set_value('account_['.$i.']', $rvd['PVD_Nomor_Rekening_Lawan']); ?>" onchange="check_rekening(<?php echo $i; ?>)" />
                            <select class="tb_col_account" id="cbo_account_<?php echo $i ?>" name="cbo_account_<?php echo $i ?>" style="display:none;" onchange="cbo_account_change(<?php echo $i; ?>)" onblur="cbo_account_blur(<?php echo $i; ?>)"><?php echo $this->account_model->get_account_for_option(false, false, true, set_value('account_['.$i.']', $pvd['PVD_Nomor_Rekening_Lawan'])); ?></select>
                            <span class="field_info tb_col_account" id="account_name_<?php echo $i; ?>"><?php echo $rvd['Account_Name']; ?></span>
                        </div>
                    </li>
                    <li class="tb_col">
                        <div class="tb_col_unit" style="display: block;  border-left: 1px dotted gray;">
                            <input maxlength="4" type="text" class="tb_col_unit" name="unit[<?php echo $i; ?>]" id="unit_<?php echo $i; ?>" value="<?php echo set_value('unit_['.$i.']', $rvd['PVD_Unit_Code_Lawan']); ?>" onchange="check_unit(<?php echo $i; ?>)" />
                            <select class="tb_col_unit" id="cbo_unit_<?php echo $i ?>" name="cbo_unit_<?php echo $i ?>" style="display:none;" onchange="cbo_unit_change(<?php echo $i; ?>)" onblur="cbo_unit_blur(<?php echo $i; ?>)"><?php echo $this->unit_model->get_unit_for_option(set_value('unit_['.$i.']', $pvd['PVD_Unit_Code_Lawan'])); ?></select>
                            <span class="field_info tb_col_unit" id="unit_name_<?php echo $i; ?>"><?php echo $rvd['Unit_Name']; ?></span>
                        </div>
                    </li>
                    <li class="tb_col">
                        <div class="tb_col_project" style="display: block;  border-left: 1px dotted gray;">
                            <input maxlength="20" type="text" class="tb_col_project" name="project[<?php echo $i; ?>]" id="project_<?php echo $i; ?>" value="<?php echo set_value('project_['.$i.']', $rvd['PVD_Project_Contract_Number_Lawan']); ?>" onchange="check_project(<?php echo $i; ?>)" />
                            <select class="tb_col_project" id="cbo_project_<?php echo $i ?>" name="cbo_project_<?php echo $i ?>" style="display:none;" onchange="cbo_project_change(<?php echo $i; ?>)" onblur="cbo_project_blur(<?php echo $i; ?>)"><?php echo $this->project_model->get_project_for_option(set_value('project_['.$i.']', $pvd['PVD_Project_Contract_Number_Lawan'])); ?></select>
                            <span class="field_info tb_col_project" id="project_name_<?php echo $i; ?>"><?php echo $rvd['PD_Description']; ?></span>
                        </div>
                    </li>
                    <li class="tb_col">
                        <div class="tb_col_identitas" style="display: block;  border-left: 1px dotted gray;">
                            <input maxlength="20" type="text" class="tb_col_identitas" name="identitas[<?php echo $i; ?>]" id="identitas_<?php echo $i; ?>" value="<?php echo set_value('identitas_['.$i.']', $rvd['PVD_Identitas_Lawan']); ?>" onchange="check_identitas(<?php echo $i; ?>)" />
                            <select class="tb_col_identitas" id="cbo_identitas_<?php echo $i ?>" name="cbo_identitas_<?php echo $i ?>" style="display:none;" onchange="cbo_identitas_change(<?php echo $i; ?>)" onblur="cbo_identitas_blur(<?php echo $i; ?>)"><?php echo $this->identitas_model->get_identitas_for_option(set_value('identitas_['.$i.']', $pvd['PVD_Identitas_Lawan'])); ?></select>
                            <span class="field_info tb_col_identitas" id="identitas_nama_<?php echo $i; ?>"><?php echo $rvd['ID_Description']; ?></span>
                        </div>
                    </li>
                    <li class="tb_col"><textarea maxlength="100" spellcheck="false" autocorrect="off" style="resize: none;" class="tb_col_keterangan" name="keterangan[<?php echo $i ?>]" id="keterangan_<?php echo $i ?>"><?php echo set_value('keterangan_['.$i.']', $rvd['PVD_Keterangan']); ?></textarea></li>
                    <li class="tb_col"><input maxlength="16" class="rupiah tb_col_nominal" type="text" name="rupiah[<?php echo $i; ?>]" id="rupiah_<?php echo $i; ?>" value="<?php echo set_value('rupiah_['.$i.']', number_format($rvd['PVD_Rupiah'], 2, '.', ',')); ?>" onblur="hitung_total()" /></li>                    
                </ul>
        <?php $i++; $total += $rvd['PVD_Rupiah']; } } ?>
